...
public class Client extends javax.swing.JFrame {
    ...
    private static String file_locate=System.getProperty("user.dir")+"\\opencv\\x64;";

    static {


        try {
            System.setProperty("java.library.path",file_locate + System.getProperty("java.library.path"));
    ...
    private static Keep_watch watch=new Keep_watch();
    ...
    private  void miniTray() { // 窗口最小化到任务栏托盘
        ...
        String filepath = rootpath + "/conf/img/Stream.png";
        ImageIcon trayImg = new ImageIcon(filepath);// 托盘图标

        trayIcon = new TrayIcon(trayImg.getImage(), "SteamGetor", new PopupMenu());
    ...
    public Client() {
        ...
        configuration_bean = BaseConfig_Tools.getConfiguration_information();
        watch.setConfiguration_bean(configuration_bean);

        Thread upload_thread=new Thread(new Serivce4upload(configuration_bean.getUploadurl(), configuration_bean.getImage_height(), configuration_bean.getImage_width(),watch));
        upload_thread.setDaemon(true);
        upload_thread.start();
        Thread watch_lookup=new Thread(watch);
        watch_lookup.setPriority(Thread.MIN_PRIORITY);
        watch_lookup.setDaemon(true);
        watch_lookup.start();
        ...
        try {
            ...
            Image logoimage = Toolkit.getDefaultToolkit().createImage(System.getProperty("user.dir")+"\\conf\\img\\Stream.png");
...
//            LogMgr.getLogMgr_instance().WirteError(e);
        ...
        this.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                int i = JOptionPane.showConfirmDialog(null, "确定要退出采集器吗？", "退出", JOptionPane.YES_NO_OPTION);
                if (i == JOptionPane.YES_OPTION) {
                    logger.info("End get stream client");
        ...
        jButtonRealPlayActionPerformed(null);
    ...
    //    static DataOutputStream yuvdataOutputStream;
//    static DataOutputStream pcmdataOutputStream;
    //ToDO
    DecCBFun decCBFun;
    static Long Currenttime=System.currentTimeMillis();
//    public  static int imyi=0;

//    public static  JFrame my_frame;

    static HCNetSDK hCNetSDK = HCNetSDK.INSTANCE;
    static PlayCtrl playControl = PlayCtrl.INSTANCE;
    static Configuration_Bean configuration_bean;
    ...
    private void initComponents() {
        ...
        jTextFieldPortNumber.setText(configuration_bean.getPort());

        jTextFieldIPAddress.setText(configuration_bean.getCamera_ip());

        jLabelPortNumber.setText("端口");

        jLabelPassWord.setText("密码");

        jPasswordFieldPassword.setText(configuration_bean.getPassword());

        jTextFieldUserName.setText(configuration_bean.getUsername());

        jButtonExit.setText("待开发");
...
//                jButtonExitActionPerformed(evt);
        ...
        jComboBoxCallback.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"回调取流", "仅预览"}));
    ...
    private void jButtonLoginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonLoginActionPerformed
        //登入之前先注销已登入的用户,预览情况下不可注销
        if (bRealPlay) {
            JOptionPane.showMessageDialog(this, "登入新用户请先停止当前取流!");
            return;
        }
        ...
        if (userID == -1) {
            ...
            JOptionPane.showMessageDialog(Client.this, "登入失败");
        } else {
            CreateDeviceTree();
        }
    ...
    {//GEN-HEADEREND:event_jButtonRealPlayActionPerformed
        ...
        if (bRealPlay == false) {
            ...
            if (previewSucValue == -1) {
                JOptionPane.showMessageDialog(this, "取流失败");
                return;
            }
    ...
    class DecCBFun implements PlayCtrl.DecCallBack {

        //    (long nPort, char * pBuf, long nSize, Client.PlayCtrl.FRAME_INFO  pFrameInfo, long nReserved1, long nReserved2)
...
//                logger.info("Audio nStamp:"+frameInfo.nStamp.intValue());
//                logger.info("test_DecCb_Write Audio16 \n");
//        if (pcmdataOutputStream==null)
//        {
////            sprintf(filename,"AudionPCM.pcm",iPicNum);
//
//            try {
//                pcmdataOutputStream = new DataOutputStream(new FileOutputStream("AudionPCM.pcm", true));
//
//                Pointer pointer=pBuffer.getPointer();
//                byte[] bugf=new byte[nSize.intValue()];
//                pointer.read(0,bugf,0,nSize.intValue());
//                pcmdataOutputStream.write(bugf);
//                pcmdataOutputStream.flush();
...
//                    try {
                ...
                if (System.currentTimeMillis() - Currenttime > configuration_bean.getSend_time_span()) {
                    Buffer_Service.inputData(new Carrier_Byte(bugf));
                    Currenttime = System.currentTimeMillis();
                }


//                        my_frame = new JFrame();
//                        my_frame.setBounds(20, 100, 1920+100, 1080+80);
//                        my_frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
//                        my_frame.getContentPane().setLayout(null);
//
...
//                        my_frame.getContentPane().add(label);
//                        label.setIcon(new ImageIcon(bufImage));
//                        my_frame.setVisible(true);


//                        if(bugf.length!=0){
//                            yuvdataOutputStream.write(bugf);
//                            yuvdataOutputStream.flush();
//                            yuvdataOutputStream.close();
//                            System.out.println("success");
//                        }else {
//                            System.out.println("failed");
//                        }
//                        yuvdataOutputStream=null;
            ...
            switch (dwDataType) {
                ...
                case HCNetSDK.NET_DVR_SYSHEAD: //系统头
                    ...
                    if (dwBufSize > 0) {
                        ...
                        if (!playControl.PlayM4_SetDecCallBackEx(m_lPort.getValue(), decCBFun, null, null)) //打开流接口
                        {
                            break;
                        }
